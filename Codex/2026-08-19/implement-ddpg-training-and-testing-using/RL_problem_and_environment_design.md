...
### 3.6 Algorithmic mechanics: TSCAC-DDPG training procedure

The implemented procedure follows most of the deterministic TSCAC mechanics. The main distinction is that the public KuaiRand-1K subset does not provide the full production candidate slate or behavior-policy propensity needed for exact NCIS, so the implementation uses a BC-ranked held-out-slate proxy for evaluation.

**Inputs.** The algorithm receives offline tuples `(s, a, r_vector, s_next, done)`, where `r_vector` contains WatchTime, Click, Like, Comment, and Anti-Hate responses. The algorithm also uses a continuous action representation for each logged video.

**Step 0 - Behavior policy estimation.** Train a supervised behavior-cloning actor `pi_beta(a|s)` to minimize the mean squared error between its predicted continuous preference action and the logged action. This supplies the BC baseline and represents the policy support of the historical log.

**Step 1 - Multi-critic initialization.** Create a separate actor, critic, target actor, and target critic for each auxiliary response:

`pi_click, pi_like, pi_comment, pi_anti_hate`

Each critic evaluates only its own response. This avoids allowing dense WatchTime to suppress the learning signal from sparse interactions.

**Step 2 - Stage-one auxiliary-policy learning.** For each auxiliary response `i` and each mini-batch:

1. Compute a target action with the target actor: `a_next = pi_i_target(s_next)`.
2. Compute the response-specific Bellman target:

   `y_i = r_i + gamma_aux * (1 - done) * Q_i_target(s_next, a_next)`

3. Update critic `Q_i` by minimizing `MSE(Q_i(s, a_logged), y_i)`.
4. Update actor `pi_i` to maximize `Q_i(s, pi_i(s))` while staying near logged actions through a BC support penalty.
5. Soft-update target networks using coefficient `tau`.

**Step 3 - Stage-two main-policy learning.** Initialize a separate WatchTime actor `pi_watch` and critic `Q_watch`. For each mini-batch:

1. Update `Q_watch` from the WatchTime Bellman target with `gamma_main`.
2. Compute the main action `a_watch = pi_watch(s)`.
3. Obtain frozen actions from all stage-one auxiliary policies.
4. Compute proximity between `a_watch` and each auxiliary action. The implementation uses mean squared distance; this is the negative-log form of the paper's Gaussian action-similarity function `h(a_i, a_watch)`.
5. Update the main actor to maximize WatchTime value while applying two protections: a behavior-support loss toward logged actions and a soft multi-auxiliary proximity constraint.
6. Soft-update the main target networks.

The implemented stage-two objective is:

`L = -Q_watch(s, pi_watch(s)) + alpha_BC * MSE(pi_watch(s), a_logged) + lambda * mean_i MSE(pi_watch(s), pi_i(s))`

`alpha_BC` limits offline extrapolation. `lambda` is the TSCAC constraint strength. Increasing `lambda` should preserve the auxiliary-response behavior more strongly, but can reduce the amount of WatchTime gain.

**Step 4 - Candidate ranking.** At inference, the main actor returns one continuous preference vector. For every candidate video `v`, calculate:

`score(v) = pi_watch(s)^T embedding(v)`

Recommend the candidate with the largest score. This follows the paper's production mechanism of mapping a continuous preference action to a discrete video recommendation with a ranking function.

**Step 5 - Constrained checkpoint selection.** Evaluate each stage-two checkpoint on the chronological validation split. Select the checkpoint with the largest constrained score, not necessarily the final checkpoint. The score starts with WatchTime and applies penalties when Click, Like, or Comment fall below the selected percentage of BC performance or when Hate rises above the BC level. The final test split is used only after checkpoint selection.

**Step 6 - Final evaluation.** Restore `best_tscac_ddpg.pt`, rank held-out candidate windows, and report `policy_*` beside `bc_*` responses. A model is considered promising only when it improves WatchTime without unacceptable auxiliary degradation.

The mechanics intentionally include target networks, response-specific critics, separate discount factors, replay mini-batches, deterministic continuous actions, two-stage actor learning, soft auxiliary constraints, behavior support, and held-out checkpoint selection. The remaining paper component to add for a closer experimental replication is NCIS or doubly robust off-policy evaluation with behavior propensities.
